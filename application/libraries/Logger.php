...
class Logger {
  ...
  public function last_log() {
    return $this->ci->db->where('id', $this->logid)->get($this->user)->row();
  }
  ...
  public function get_num() {
    $this->_getQueryMaker();
    return $this->ci->db->from($this->user)->count_all_results();
  }

  public function get() {
    $this->_getQueryMaker();
    $result = $this->ci->db->get($this->user);
    return $this->_dbcleanresult($result);
  }

  public function remove_log() {
    $this->_getQueryMaker();
    $this->ci->db->delete($this->user);
  }

  public function get_ids() {
    $this->_getQueryMaker();
    $ids = $this->ci->db->select('type_id')->get($this->user)->result_array();
